# postgres-13-backup-s3

Backup PostgreSQL 13 to S3 (supports periodic backups)

## Usage

Docker:
```sh
$ docker run -e S3_ACCESS_KEY_ID=key -e S3_SECRET_ACCESS_KEY=secret -e S3_BUCKET=my-bucket -e S3_PREFIX=backup -e POSTGRES_DATABASES=dbname -e POSTGRES_USER=user -e POSTGRES_PASSWORD=password -e POSTGRES_HOST=localhost dzangolab/postgres-13-backup-s3
```
...
pgbackups3:
  image: dzangolab/postgres-13-backup-s3
  links:
    - postgres
  environment:
    SCHEDULE: '@daily'
