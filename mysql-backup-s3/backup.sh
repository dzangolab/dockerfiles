...
if [ "${DATABASES}" = "ALL" ]; then
  ...
  DB_LIST=$(printf '%s' "${DATABASES}" | tr ',' ' ')
fi

for DB in $DB_LIST; do
  DUMP_FILE=dump.sql
  SRC_FILE="${DUMP_FILE}.gz"
  ...
  if ! mysqldump $DATABASE_HOST_OPTIONS $MYSQLDUMP_OPTIONS "$DB" > "$DUMP_FILE"; then
  ...
  if ! gzip "$DUMP_FILE"; then
    >&2 echo "Error compressing dump for database: ${DB}"
    rm -f "$DUMP_FILE" "$SRC_FILE"
    continue
  fi
