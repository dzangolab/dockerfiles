...
assert_unset() {
  ...
  if [ -z "$current" ]; then
    ...
    echo "FAIL: $desc ($var is set to '$current')"
    failures=$((failures + 1))
  fi
}
...
OTHER_VAR_FILE=/etc/passwd
export OTHER_VAR_FILE
expand_secrets
...
MISSING_FILE=/run/secrets/does-not-exist
export MISSING_FILE
expand_secrets
